...
impl Players {
    ...
    pub fn get_from_addr(&self, addr: SocketAddr) -> Option<&Player> {
    ...
    pub fn get_from_addr_mut(&mut self, addr: SocketAddr) -> Option<&mut Player> {
    ...
    /// # Returns
    /// An option containing a mutable reference to the matching `Player`, or `None` if not found.
    pub fn get_from_nickname(&mut self, nickname: &str) -> Option<&Player> {
        for player in &mut self.player {
